...
import {
  ...
  Presentation, FileSpreadsheet, ArrowRight, ExternalLink,
} from "lucide-react";
    ...
    <Shell>
      <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
        <div>
          ...
          <h1 className="text-5xl font-bold tracking-tight mt-3">Content Studio</h1>
          <p className="text-ink-muted mt-2 text-lg">Agent-generated social posts and downloadable reports.</p>
        </div>
        {phaseBRunning && (
      ...
      <Tabs defaultValue="social" className="w-full">
        <TabsList className="bg-ink-surface border border-ink-border p-1 h-auto">
          <TabsTrigger data-testid="tab-social" value="social" className="data-[state=active]:bg-ink-elevated data-[state=active]:text-ink-text text-ink-muted px-6 text-base">Social Media Content</TabsTrigger>
          <TabsTrigger data-testid="tab-reporting" value="reporting" className="data-[state=active]:bg-ink-elevated data-[state=active]:text-ink-text text-ink-muted px-6 text-base">Reporting</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="social" className="mt-6 space-y-8">
          {content.positioning_line && (
            <div className="rounded-2xl border border-ink-border bg-ink-surface p-6">
              <div className="text-xs uppercase tracking-wider text-ink-muted mb-1.5">Positioning line</div>
              <p className="text-xl font-bold text-ink-text leading-snug">{content.positioning_line}</p>
              {content.messaging_pillars?.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {content.messaging_pillars.map((p) => <span key={p} className="text-sm px-3 py-1 rounded-full bg-ink-elevated border border-ink-border">{p}</span>)}
                </div>
              )}
            </div>
          )}
...
function Badge({ children }) {
  return <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border border-brand-accent/40 bg-brand-accent/10 text-brand-accent"><Sparkles className="w-3 h-3" /> {children}</span>;
}
    ...
    <div className="min-h-screen bg-ink-bg">
      ...
      <main className="max-w-[1400px] mx-auto px-6 lg:px-10 py-8">
        <div className="flex items-center gap-2 text-sm text-ink-muted mb-6">
          ...
          <Link to="/command-center" className="hover:text-ink-text transition-colors">Strategy Studio</Link>
          <ChevronRight className="w-3 h-3" />
          <span className="text-ink-text">Content</span>
        </div>
        {children}
      </main>
    </div>
...
function Empty({ title, desc, cta, onClick }) {
  return (
    <div className="rounded-2xl border border-dashed border-ink-border bg-ink-surface/30 p-12 text-center" data-testid="studio-empty">
...
function ContentGroup({ title, Icon, items, emptyText, running = false }) {
  if (!items || items.length === 0) {
    if (emptyText) return (
      <div>
        <div className="flex items-center gap-2 mb-3"><Icon className="w-5 h-5 text-brand-primary" /><h3 className="text-lg font-bold">{title}</h3></div>
        <div className="rounded-2xl border border-dashed border-ink-border p-6 text-sm text-ink-muted text-center flex items-center justify-center gap-2">
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-3"><Icon className="w-5 h-5 text-brand-primary" /><h3 className="text-lg font-bold">{title}</h3><span className="text-sm text-ink-muted">({items.length})</span></div>
      <div className="space-y-3">
        {items.map((it) => (
          <motion.div key={it.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-ink-border bg-ink-surface overflow-hidden">
            <div className="px-6 py-4 border-b border-ink-border bg-ink-bg/30 flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="font-semibold text-base text-ink-text">{it.heading}</div>
                {it.meta && <div className="text-sm text-ink-muted">{it.meta}</div>}
              </div>
              <Button onClick={() => { navigator.clipboard?.writeText(it.body || ""); toast.success("Copied to clipboard"); }} data-testid={`copy-${it.id}`} variant="ghost" size="sm" className="text-ink-muted hover:text-ink-text hover:bg-ink-elevated h-9 shrink-0">
                <Copy className="w-4 h-4 mr-1.5" /> Copy
              </Button>
            </div>
            <div className="px-6 py-5">
              <pre className="text-base text-ink-text leading-relaxed whitespace-pre-wrap font-sans">{it.body}</pre>
              {it.footer && <div className="mt-4 pt-4 border-t border-ink-border text-sm text-ink-muted">{it.footer}</div>}
            </div>
          </motion.div>
        ))}
      </div>
